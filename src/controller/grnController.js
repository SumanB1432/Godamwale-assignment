    ...
    try {
        ...
        if (!getInvoice) {
            return res.status(404).send({
                status: false,
                message: `${invoiceNumber} is not found`
            })
        }
        let getInvoiceFromGrn = await grnModel.findOne({
            invoiceNumber: invoiceNumber,
            isDeleted: false
        });
        ...
        if (!isValid(vendorAddress)) {
            return res.status(400).send({
                status: false,
                message: "please provide vendor address"
            })
        }
        if (!isValid(grnLineItems)) {
            return res.status(400).send({
                status: false,
                message: "please provide grnListitem"
            })
        }
        if (!mongoose.isValidObjectId(grnLineItems)) {
            return res.status(400).send({
                status: false,
                message: `${grnLineItems} is not valid`
            })
        }
        ...
        if (!getGrnLineItem) {
            return res.status(404).send({
                status: false,
                message: `${grnLineItems} is not found`
            })
        }
...
const getGrn = async function (req, res) {
    try {
        ...
        if (!grnId) {
            return res.status(400).send({
                status: false,
                message: "please provide GRN id"
            })
        }
        if (!mongoose.isValidObjectId(grnId)) {
            return res.status(400).send({
                status: false,
                message: `${grnId} is not valid`
            })
        }

        let getGrn = await grnModel.findOne({
            _id: grnId,
            isDeleted: false
        });
...
const updateGrn = async function (req, res) {
    try {
        ...
        if (status == 'GENERATED') {
            return res.status(400).send({
                status: false,
                message: "GRN is already Generated"
            })
        }

        if (status == "CANCELED") {
            let cancleGrn = await grnModel.findOneAndUpdate({
                _id: grnId
            }, {
                isDeleted: true,
                status: status
        ...
        if (status == "COMPLETED") {
            ...
            let completedGrn = await grnModel.findOneAndUpdate({
                _id: grnId
            }, {
                status: status
            }, {
