    ...
    try {
        ...
        if (!Object.keys(data).length) {
            return res.status(400).send({
                status: false,
                message: 'please provide data to create item'
            })
        }
        ...
        if (!(/^[0-9]+$/).test(quantity)) {
            return res.status(400).send({
                status: false,
                message: "Puantity should be in number formate "
            })
        }
        if (!isValid(stockPrice)) {
            return res.status(400).send({
                status: false,
                message: "please provide stock price"
            })
        }
        if (!(/^\d+(,\d{1,2})?$/).test(stockPrice)) {
            return res.status(400).send({
                status: false,
                message: "stock price should be in number formate "
            })
        }
        ...
        if (!isValid(sellprice)) {
            return res.status(400).send({
                status: false,
                message: "please provide sellprice"
            })
        }
...
const getItem = async function (req, res) {
    try {
        let getProduct = await itemModel.find({
            isDeleted: false
        });
        if (!getProduct) {
