...
const createGrnItem = async function (req,res){
    try {
        ...
        let getItem = await itemModel.findOne({productname:productName,isDeleted:false});
        if(!getItem){
            return res.status(404).send({status:false,message:`${productName} is not found`})
        }

        if(!isValid(quantity)){
            return res.status(400).send({status:false,mesage:"please provide quantity"})
        }
        ...
        return res.status(201).send({status:true,message:"successfull",data:creategrnLineItem})
